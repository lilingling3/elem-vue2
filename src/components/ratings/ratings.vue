      ...
      <div class="evaluation">
        ...
        <div class="classify">
          <span v-for="(item,index) in classifyArr" class="item"
            :class="{'active':item.active,'bad':index==2,'badActive':item.active&&index==2}" @click="filterEvel(item)">
...
export default {
  ...
  computed:{
    evelArr(){
      ...
      // console.log("selectIndex：选择类型 "+selectIndex)
      // console.log('this.evelflag 只看内容 '+this.evelflag)
      // filter 返回为true 的元素组成一个新的数组
      return selectIndex?
       this.ratings.filter((data) => this.evelflag ? data.rateType === selectIndex-1 && data.text : data.rateType === selectIndex -1)
      :this.ratings.filter((data) => this.evelflag ? data.text : true) // selectIndex 为0 表示是第一个
    }
  },
  methods:{
    _init(){
